...
DUK_LOCAL duk_uint32_t duk__push_tval_to_property_key(duk_context *ctx, duk_tval *tv, duk_hstring **out_h) {
	...
	if (DUK_TVAL_IS_STRING(tv)) {
		...
		/* Slow path, untypical. */
		duk_to_primitive(ctx, -1, DUK_HINT_STRING);  /* needed for e.g. Symbol objects */
		tv_tmp = DUK_GET_TVAL_NEGIDX(ctx, -1);
		if (DUK_TVAL_IS_STRING(tv_tmp)) {
			/* ES6 ToPropertyKey() just special cases symbols
			 * (in ES6 Section 7.1.14, step 3), but since normal
			 * strings are no-ops for ToString(), we can also
			 * catch them here.
			 */
			h = DUK_TVAL_GET_STRING(tv_tmp);
		} else {
